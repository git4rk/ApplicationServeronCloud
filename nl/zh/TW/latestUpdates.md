...
* 已解決 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 的[數個安全漏洞]( http://www.ibm.com/support/docview.wss?uid=swg22010172){: new_window}，包括：
  * IBM® Java SDK 中的多個漏洞
  * IBM WebSphere Application Server 在使用「管理主控台」時可能提供比預期弱之安全的漏洞。
...
* 已在達拉斯、倫敦及雪梨新增「WebSphere Application Server in IBM Cloud 移轉特性」。此特性提供從現有第 7 版、第 8 版或 8.5.5 版內部部署環境到 IBM Cloud 中執行之第 9 版環境的完整端對端移轉。如需相關資訊，請參閱 [WebSphere Configuration Migration Tool for IBM Cloud](https://developer.ibm.com/wasdev/downloads/#asset/tools-WebSphere_Configuration_Migration_Tool_for_IBM_Cloud)。
* 整合式細項服務維護。
...
* 已解決 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 的[數個安全漏洞](http://www-01.ibm.com/support/docview.wss?uid=swg22010172){: new_window}，包括：
  ...
  * IBM WebSphere Application Server 可能提供比使用「管理主控台」更新 Web 服務安全連結設定之後的預期安全還要弱之安全的漏洞。
  * IBM WebSphere Application Server 9.0.0.4 版可能提供比使用 PasswordUtil 指令啟用 AES 密碼加密之後的預期安全還要弱之安全的漏洞。
...
* 整合式細項服務維護。
* 已升級 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔，因此，修正套件 8.5.5.12 或 9.0.0.4 會與新的傳統 WebSphere Application Server 實例一起安裝。
* 已升級 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔，因此，修正套件 17.0.0.2 會與新的 WebSphere Application Server Liberty（Core 及 ND 方案）實例一起安裝。
* 已新增[進階 VPN 配置管理](https://console.bluemix.net/docs/services/ApplicationServeronCloud/networkEnvironment.html#advancedVPN){: new_window}特性，可讓您在倫敦及雪梨地區建立及管理多個 VPN 配置。
* 已新增[公用網際網路存取](https://console.bluemix.net/docs/services/ApplicationServeronCloud/networkEnvironment.html#publicInternetAccess){: new_window}特性的加強功能，容許用戶端更妥善管理其公用 IP 位址。
* 已解決 IBM SDK Java Technology Edition 中會影響 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 運行的[數個安全漏洞](http://www-01.ibm.com/support/docview.wss?uid=swg22002169){: new_window}，包括：
  * 與 Java SE AWT 元件相關的未指定漏洞，可能容許未經鑑別的攻擊者控制系統。
  * 與 Java SE JCE 元件相關的未指定漏洞，可能容許未經鑑別的攻擊者控制系統。
  * 與 Java SE JAXP 元件相關的未指定漏洞，可能容許未經鑑別的攻擊者使用不明的攻擊向量來進行會導致高可用性影響的拒絕服務。
  * 與「Java SE 網路」元件相關的未指定漏洞，可能容許未經鑑別的攻擊者導致低機密性影響、低完整性影響，且沒有可用性影響。
  * 與「Java SE 網路」元件相關的未指定漏洞，可能容許未經鑑別的攻擊者導致無機密性影響、低完整性影響，且沒有可用性影響。
  * 與「Java SE 安全」元件相關的未指定漏洞，可能容許未經鑑別的攻擊者導致無機密性影響、低完整性影響，且沒有可用性影響。
...
* 整合式細項服務維護。
* 已新增[進階 VPN 配置管理](https://console.bluemix.net/docs/services/ApplicationServeronCloud/networkEnvironment.html#advancedVPN){: new_window}特性，可讓您在法蘭克福地區建立及管理多個 VPN 配置。
* 已升級 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔，因此 JDK 8 會與所有新的服務實例一起安裝。
* 已解決 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 的[數個安全漏洞](http://www-01.ibm.com/support/docview.wss?uid=swg22004264){: new_window}，包括：
  ...
  * 與 Libraries 元件相關的未指定漏洞，容許遠端攻擊者使用不明的攻擊向量來取得會導致高機密性影響的機密性資訊。
...
* 已解決 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 的[數個安全漏洞](http://www-01.ibm.com/support/docview.wss?uid=swg22000834){: new_window}，包括：
  ...
  * WebSphere Application Server 可能提供比預期安全還要弱之安全的漏洞。遠端攻擊者可能會惡意探索此弱點來取得機密性資訊，並且取得管理主控台的未獲授權存取。
...
* 整合式細項服務維護。
* 已升級 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔，因此，修正套件 8.5.5.11 或 9.0.0.3 會與新的傳統 WebSphere Application Server 實例一起安裝。
* 已解決 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 的[數個安全漏洞](https://www-01.ibm.com/support/docview.wss?uid=swg22000587){: new_window}，包括：
  * 與 Libraries 元件相關的未指定漏洞，沒有機密性影響、高完整性影響，且沒有可用性影響。
  * 與 Libraries 元件相關的未指定漏洞，容許遠端攻擊者使用不明的攻擊向量來取得會導致高機密性影響的機密性資訊。
  * 與 Libraries 元件相關的未指定漏洞，容許遠端攻擊者使用不明的攻擊向量來進行會導致低可用性影響的拒絕服務。
  * OpenSSL 中的漏洞，容許遠端攻擊者取得機密性資訊，這是 DES/3DES 密碼錯誤所導致，用於 SSL/TLS 通訊協定中。
  * 一個漏洞，容許使用者在 Web 使用者介面中內嵌任意 JavaScript 程式碼，因此可能會變更想要的功能，而導致在授信階段作業內揭露認證。
...
* 整合式細項服務維護。
* 已升級 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔，因此，修正套件 8.5.5.11 或 9.0.0.2 會與新的傳統 WebSphere Application Server 實例一起安裝。
* 已升級 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔，因此，修正套件 16.0.0.4 會與新的 WebSphere Application Server Liberty（Core 及 ND 方案）實例一起安裝。
...
* 已解決 IBM SDK Java Technology Edition 中會影響 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 運行的[數個安全漏洞](https://www-01.ibm.com/support/docview.wss?uid=swg21995995){: new_window}，包括：
  ...
  * Oracle Java SE 及 Java SE Embedded 中與 Networking 元件相關的未指定漏洞，容許遠端攻擊者使用不明的攻擊向量來取得會導致高機密性影響的機密性資訊。
  *  IBM WebSphere Application Server 中的跨網站 Scripting 漏洞，容許使用者在 Web 使用者介面中內嵌任意 JavaScript 程式碼，因此可能會變更想要的功能，而導致在授信階段作業內揭露認證。
...
* 已升級 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔，因此，新的 WebSphere Application Server Liberty（Core 及 ND 方案）實例都會安裝修正套件 16.0.0.3。
...
* 已將 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔從 8.5.5.9 升級到 8.5.5.10
...
* 已升級 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔，因此，新的 WebSphere Application Server Liberty（Core 及 ND 方案）實例都會安裝修正套件 16.0.0.2。16.0.0.2 是 8.5.5.9 之後的下一個修正套件。從 16.0.0.2 開始，預設會安裝為這些方案支援的所有授權 Liberty 選購配件。
...
* 已將 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔從 8.5.5.8 升級到 8.5.5.9
...
* 已將 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔從 8.5.5.7 升級到 8.5.5.8
...
* 已將 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 二進位檔從 8.5.5.6 升級到 8.5.5.7
