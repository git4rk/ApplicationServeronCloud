...
대부분의 경우 **VPN 구성 다운로드** 단추를 사용하여 다운로드할 수 있는 단일 VPN 구성만 필요합니다. 그러나 {{site.data.keyword.Bluemix_notm}} UI의 서비스 대시보드에서 **고급 VPN 관리** 단추를 사용하여 액세스할 수 있는 고급 VPN 관리 페이지에서 여러 VPN 구성을 작성하고 관리할 수 있습니다. 구성이 여러 개인 경우 이전의 한 VPN 구성이 거의 만료될 때 새 구성으로 원활하게 전환하는 데 도움이 될 수 있습니다. 사용자 조직에서 서로 다른 개인이나 팀 단위로 VM에 대한 액세스를 관리하도록 여러 VPN 구성을 요청할 수도 있습니다.  
...
공인 IP 주소를 **요청**하고 연결을 **열려면** 다음 지시사항을 따르십시오.
...
2. 호스트의 IP 주소가 표시되지만 이는 공인 IP 주소가 아닙니다. **공인 IP 주소 요청**을 클릭하십시오.
    ...
    **참고:** 공인 IP가 지정된 서비스 대시보드로 돌아갑니다, 그러나 다음 메시지가 표시됩니다. 

    > **현재 액세스가 닫혀 있습니다. 공인 IP 관리를 클릭하여 액세스를 여십시오.**
...
4. 호스트의 IP 주소와 새 공인 IP가 표시되지만 액세스는 닫힙니다. **액세스 열기**를 클릭하십시오.
    ...
    **참고:** 다음 메시지가 표시된 서비스 대시보드로 돌아갑니다, 

    > **현재 액세스가 열려 있습니다. 공인 IP 관리를 클릭하여 액세스를 닫으십시오.**

연결을 **닫고** 공인 IP 주소를 **리턴**하려면 다음 지시사항을 따르십시오.
...
4. **공인 IP 주소 리턴**을 클릭하십시오.
    ...
    **참고:** 호스트의 IP 주소와 다음 메시지가 표시된 서비스 대시보드로 돌아갑니다, 
...
공인 IP에 대한 액세스를 열 때는 IP 주소가 사용자의 VM과 연관되며 포트 80 및 443이 게이트웨이에서 열립니다. 그러나 기본적으로 Liberty Core 및 Traditional WebSphere 기본 서버는 포트 80 및 443을 열지 않습니다. 반대로 포트 80 및 443이 IBM HTTP Server에서 기본적으로 열립니다. 따라서 공인 IP를 사용할 때는 Liberty Core 및 Traditional WebSphere 기본 서버가 포트 80/443에서 애플리케이션 트래픽을 청취하도록 구성해야 할 수 있습니다.
...
WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} VM은 두 개의 DNS 분석기를 사용하여 구성된다는 것을 유의해야 합니다. 분석기는 VM의 **/etc/resolv.conf** 파일에 구성됩니다. 기본 DNS 서버는 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} 서비스에서 제공한 권한 없는 서버입니다. 보조 DNS 서버는 {{site.data.keyword.Bluemix_notm}}에서 제공한 권한 없는 서버입니다.

사용자 요구에 맞는지 확인하려면 DNS 구성을 검토할 것을 권장합니다. IBM에서 제공한 항목이 사용자 요구사항을 충족하지 않는 경우 사용자가 선택한 DNS 서버를 참조하도록 **/etc/resolv.conf** 파일을 업데이트할 수 있습니다.

**참고:** 이전 WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} VM에서는 **/etc/resolv.conf** 파일에 하나의 기본 DNS 서버만 구성할 수 있었습니다. HA 솔루션을 원하는 경우 VM을 해제하고 새 VM을 프로비저닝하거나 **/etc/resolv.conf** 파일을 업데이트하여 보조 DNS 서버를 추가할 수 있습니다. 이는 선호하는 DNS 서버이거나 {{site.data.keyword.Bluemix_notm}}(10.0.80.11)에서 제공하는 DNS 서버일 수 있습니다.


## 사용자 정의 노드에서 새 서버에 대한 포트 열기
{: #serversOnCustom}

사용자 정의 노드에서 서버를 작성하는 경우 서버를 시작하기 전에 서버에 필요한 추가 포트가 배치 관리자 또는 사용자 정의 노드 VM에 열려 있어야 합니다. 서버를 작성한 후 그러나 서버를 시작하기 전에 `openWASPorts.sh` 스크립트를 실행하여 포트를 여십시오.
