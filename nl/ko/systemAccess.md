...
이 절에서는 시스템에 액세스하고 액세스를 설정하는 다양한 방법과 함께 서비스 인스턴스 작성 및 관리 방법을 설명합니다.
{: shortdesc}
...
**참고:** [REST API 샘플](https://github.com/IBM-Cloud/WebSphere-in-Bluemix/tree/master/WebSphere-In-Bluemix-API-Examples){: new_window}에서 참조되는 **apiEndpoint** URL은 미국 남부 지역을 가리킵니다. 다른 지역을 사용 중인 경우 애플리케이션에서 적합한 **apiEndpoint**를 참조하는지 확인하십시오.
...
구성을 캐시하고 나면 Tunnelblick에서 액세스할 수 있습니다. 쉽게 액세스하려면 맨 위 메뉴 표시줄에 아이콘을 둘 수 있습니다.
...
5. 다음 명령을 사용하여 **virtuser**를 **root**로 전환해서 전체 sysadmin 권한을 얻으십시오.
...
9. **virtuser**로 연결하려면 "ssh VM1"을 실행하십시오.
...
* Traditional WebSphere Application Server 명령은 */opt/IBM/WebSphere/Profiles/Default**profile_typeprofile_number**/bin*의 기본 프로파일 위치에서 실행할 수 있습니다.
