...
Depois de provisionar um WebSphere Application Server na instância de serviço do {{site.data.keyword.Bluemix_notm}} no painel de serviço na IU do {{site.data.keyword.Bluemix_notm}}, é possível estabelecer uma conexão OpenVPN. Para criar a conexão, expanda o menu suspenso e clique em
**Fazer download da configuração de VPN** para fazer download de sua configuração de VPN. A configuração de VPN contém um arquivo `.ovpn` e certificados que são usados para autenticar com o servidor OpenVPN. Depois que a conexão do OpenVPN for estabelecida, será possível acessar a sua VM por meio de SSH. Também
...
Na maioria dos casos, é necessária apenas uma única configuração de VPN que possa ser transferida por download usando o botão **Download da configuração de VPN**. No entanto, a página de gerenciamento de VPN avançado, que é acessada clicando em **Gerenciamento de VPN avançado** no painel de serviço, permite criar e gerenciar diversas configurações de VPN. Ter múltiplas configurações pode ser útil para fazer a transição suavemente para uma nova configuração de VPN quando a antiga estiver prestes a expirar. Também é possível solicitar múltiplas configurações de VPN para gerenciar o acesso às suas VMs com indivíduos ou equipes diferentes em sua organização.  
...
Opcionalmente, é possível gerenciar o acesso à Internet pública por meio do painel de serviço na IU do {{site.data.keyword.Bluemix_notm}}. É possível **solicitar** um endereço IP público do conjunto e **abrir** a conexão da Internet com o WebSphere Application Server na instância de serviço do {{site.data.keyword.Bluemix_notm}}. Por outro lado, é possível **fechar** o acesso de sua instância de serviço para a Internet e **retornar** o endereço IP público para o conjunto.

### Solicitar um endereço IP público e abrir uma conexão
{: #request-open-ip}

1. Clique em **Gerenciar acesso de IP público** no painel de serviço no console do {{site.data.keyword.Bluemix_notm}}.
2. O endereço IP para o host é mostrado, mas não o seu endereço IP público. Clique em **Solicitar endereço IP público**.

    Você será retornado para o painel de serviço com um IP público designado. No entanto, a mensagem a seguir é exibida:

    > _O acesso está atualmente fechado. Clique em Gerenciar IP público para abrir acesso._
3. Clique em **Gerenciar acesso de IP público** no painel de serviço.
4. Os endereços IP para o seu host e o seu novo IP público são mostrados, mas o acesso é fechado. Clique em  ** Abrir Acesso **.

    Você será retornado para o painel de serviço com a mensagem exibida a seguir:

    > _Atualmente o acesso está aberto. Clique em Gerenciar IP público para fechar acesso. _

### Fechar uma conexão e retornar um endereço IP público
{: #close-return-ip}

1. Clique em **Gerenciar acesso de IP público** no painel de serviço.
2. Clique em **Fechar acesso**.

    Você será retornado para o painel de serviço com a mensagem exibida a seguir:

    > _O acesso está atualmente fechado. Clique em Gerenciar IP público para abrir acesso._
3. Clique em **Gerenciar acesso de IP público** no painel de serviço na IU do {{site.data.keyword.Bluemix_notm}}.
4. Clique em **Retornar endereço IP público**.

    Você será retornado para o painel de serviço no qual o endereço IP de seu host é mostrado com a mensagem a seguir exibida:

    > _IP público retornado._
...
É necessário configurar o Liberty Core para atender ao tráfego de aplicativo nas portas 80 e 443 ao usar o IP público.
...
É importante observar que o WebSphere Application Server em VMs do {{site.data.keyword.Bluemix_notm}} são configurados com dois resolvedores de DNS. Os resolvedores são configurados no arquivo **/etc/resolv.conf** na VM. O servidor DNS primário é um servidor não autorizado que é fornecido pelo WebSphere Application Server no serviço do {{site.data.keyword.Bluemix_notm}}. O servidor DNS secundário é um servidor não autorizado fornecido pelo {{site.data.keyword.Bluemix_notm}}.

Nós recomendamos que você revise a configuração de DNS para ver se ela se ajusta às suas necessidades. É possível atualizar o arquivo `/etc/resolv.conf` para referenciar o servidor DNS de sua escolha se os fornecidos pela IBM não atenderem aos seus requisitos.

**Nota:** o WebSphere Application Server mais antigo em VMs do {{site.data.keyword.Bluemix_notm}} pode ter apenas um servidor DNS primário configurado no arquivo `/etc/resolv.conf`. Se desejar uma solução de alta disponibilidade, libere a VM e forneça uma nova ou atualize o arquivo `/etc/resolv.conf` para incluir um servidor DNS secundário. O servidor DNS secundário pode ser seu servidor DNS preferencial ou aquele fornecido pelo {{site.data.keyword.Bluemix_notm}} (10.0.80.11).


## Abrindo portas para novos servidores em nós customizados
{: #serversOnCustom}

Ao criar um servidor em um nó customizado, as portas adicionais que são necessárias pelo servidor devem ser abertas no gerenciador de implementação e nas VMs do nó customizado antes de iniciar o servidor. Depois de criar o servidor, mas antes de iniciar o servidor, abra as portas executando o script
`openWASPorts.sh`.
