...
Os métodos de criação e gerenciamento de uma instância de serviço são discutidos nesta seção, juntamente com várias maneiras
de acessar e configurar o acesso a seus sistemas.
{: shortdesc}
...
Por meio do uso de APIs REST compatíveis com o Swagger 2.0, os clientes têm acesso à mesma função conforme fornecido no portal e no painel. Para obter mais informações sobre APIs de REST e recursos suportados, consulte a {{site.data.keyword.Bluemix_notm}}[Documentação de API de REST do WebSphere Application Server](https://wasaas-broker.ng.bluemix.net/wasaas-broker/api#/){: new_window}. Para o código de amostra que demonstra o uso das APIs de REST, faça download das {{site.data.keyword.Bluemix_notm}} [Amostras de API de REST](https://github.com/IBM-Cloud/WebSphere-in-Bluemix/tree/master/WebSphere-In-Bluemix-API-Examples){: new_window} do WebSphere Application Server hospedado no Git.

**Nota:** depois de criar uma instância de serviço, dependendo do tamanho da camiseta que é criada, o seu
serviço pode não estar imediatamente pronto para uso. Recomenda-se consultar o campo **Status** do JSON retornado para determinar o estado atual da instância de serviço.

**Nota:** a URL **apiEndpoint** referenciada em [Amostras de API REST](https://github.com/IBM-Cloud/WebSphere-in-Bluemix/tree/master/WebSphere-In-Bluemix-API-Examples){: new_window} aponta para a região sul
...
3. Conecte-se à VPN e, em seguida, poderá acessar sua máquina virtual. Após seu primeiro acesso, o Tunnelblick armazena a configuração em cache e você pode se conectar a partir do Tunnelblick. É possível colocar um ícone na barra de menus para fácil acesso.
...
Use as instruções a seguir para configurar o acesso SSH ao seu WebSphere Application Server nas VMs do {{site.data.keyword.Bluemix_notm}}
...
5. Obtenha a autoridade completa do sysadmin ao alternar **virtuser** para **root** usando o comando:
...
* Os comandos do WebSphere Application Server tradicional podem ser emitidos do local do perfil padrão em * /opt/IBM/WebSphere/Profiles/Default**profile_typeprofile_number**/bin*, em que:
...
bancos de dados.
  * Em cada WebSphere Application Server no nó do {{site.data.keyword.Bluemix_notm}}, você localiza um script openFirewallPorts.sh no diretório WAS_HOME/virtual/bin.
