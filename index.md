...
**Note:** Consumers are now able to choose between the current fixpack level or one older version [(n or n-1)](/docs/ApplicationServeronCloud?topic=wasaas-updating-your-environment#maintenance-strategy){: new_window} when you create any WebSphere Application Server in {{site.data.keyword.Bluemix_notm}} instance.
...
The following table shows the proration rates for each plan and the effective price per actual block hour after the proration is calculated. For current prices in your region, [contact IBM Sales](/docs/ApplicationServeronCloud?topic=wasaas-reporting_issues#contacting-sales).
